...
void run_strength_test(
    ...
    std::cout << "  New Model Player:  Cycling (RED->BLUE->YELLOW->GREEN->...)" << std::endl; // Clarify cycling
    ...
    try {
        ...
        std::cerr << "Error loading old model: " << e.what() << std::endl;
        return;
    }
    ...
    for (int game_idx = 0; game_idx < num_games; ++game_idx) {
        ...
        // --- Analyze Result ---
        // Use board.get_winner() which determines the single winner based on max score (with implicit tie-breaking)
        std::optional<Player> winner = board.get_winner(); // This internally calls get_game_result()

        bool new_model_won = false;
        if (winner.has_value() && winner.value() == new_model_player) {
            new_model_wins++;
            new_model_won = true; // Flag for logging if needed
        }
    ...
    double win_percentage = (num_games > 0) ? (static_cast<double>(new_model_wins) / num_games * 100.0) : 0.0;
    std::cout << "New Model Wins: " << new_model_wins << "/" << num_games
              << " (" << std::fixed << std::setprecision(2) << win_percentage << "%)" << std::endl;
    if (num_games > 0) {
